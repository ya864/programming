...
class Map:
    ...
    move_x = {'N':0, 'NE': -1, 'E':1, 'ES': 1, 'S':0, 'SW': -1, 'W':-1, 'WN': 1}
    move_y = {'N':-1,'NE': -1, 'E':0, 'ES': 1, 'S':1, 'SW': 1, 'W':0, 'WN': -1}
...
for i in range(n * 2):
    ...
    turns.append({'x': x, 'y': y})

stone = '';
while len(turns) > 0:
    turn = turns.pop(0)
    stone = 'A' if len(turns) % 2 == 1 else 'B'
    field.setPoint(turn['x'], turn['y'], stone)
    for i in range(len(directions)):
        if field.checkThereStone(turn['x'], turn['y'], directions[i], stone) == True:
            field.setPointLoop(turn['x'], turn['y'], directions[i], stone)
